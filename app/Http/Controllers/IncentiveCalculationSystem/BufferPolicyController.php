...
class  BufferPolicyController extends Controller
{
    var $authorize = null;
    ...
    {
      //dd($request);
      if($this->authorize->hasPermission('BUFFER_POLICY_CREATE'))//check permission
      {
    ...
    {
      if($this->authorize->hasPermission('BUFFER_POLICY_VIEW'))//check permission
      {
    ...
    {
      if($this->authorize->hasPermission('BUFFER_POLICY_EDIT'))//check permission
      {
    ...
    {
      if($this->authorize->hasPermission('BUFFER_POLICY_DELETE'))//check permission
      {
        //   $is_exists=DB::table('style_creation')->where('product_category_id',$id)->exists();
        // if($is_exists==true){
        //   return response([ 'data' => [
    ...
    private function datatable_search($data)
    {
      if($this->authorize->hasPermission('BUFFER_POLICY_VIEW'))//check permission
      {
        ...
        $type_list = BufferPolicy::select('*')
        ->where('hours'  , 'like', $search.'%' )
        ->orderBy($order_column, $order_type)
        ->offset($start)->limit($length)->get();

        $type_count = BufferPolicy::where('hours'  , 'like', $search.'%' )->count();
