...
class SAPService
{
    ...
    private function loginRequest(){
        ...
        $this->sessionId = $resArray->SessionId;
    ...
    public function patchRequest($object,$doc_entry,$body){
        ...
        curl_setopt($this->curl, CURLOPT_HTTPHEADER, $this->headers);
        curl_setopt($this->curl, CURLOPT_URL, $this->hanaHost . ":" . $this->port.$this->version."".$object."($doc_entry)");
        curl_setopt($this->curl, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($this->curl, CURLOPT_RETURNTRANSFER, true);
