  ...
  public static function generateUniqueId($type , $company_id)
  {
      $unque_id = DB::transaction(function () use ($type , $company_id){
          ...
          if($company_id == null || $company_id == 0){ //for master data
            DB::table('inc_unique_id_generator')
            ->where([ ['process_type' , '=' , $type]])
            ->increment('unque_id', 1);

            $id = DB::table('inc_unique_id_generator')->where([['process_type' , '=' , $type]])
            ->sharedLock()
            ->value('unque_id');
            ...
            $id = DB::table('inc_unique_id_generator')->where([ ['process_type' , '=' , $type] , ['company' , '=' , $company_id] ])
            ->sharedLock()
            ->value('unque_id');
